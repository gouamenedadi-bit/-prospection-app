...
const FALLBACK_AVIS: PublicAvis[] = [
  {
    rating: 5,
    comment: "Depuis que j'utilise l'application, mes clients trouvent mon bureau en quelques secondes, même dans les quartiers qu'ils ne connaissent pas.",
    displayName: "Aya K.",
    role: "Prospection à Cocody",
  },
  {
    rating: 5,
    comment: "Je vois enfin les vrais prix avant de me déplacer. Plus besoin d'appeler pour vérifier — ça change tout au quotidien.",
    displayName: "Issa T.",
    role: "Partenaire à Bouaké",
  },
  {
    rating: 4,
    comment: "La partie Soins & bien-être m'aide à mieux conseiller mes clients, et à leur proposer directement le bon produit.",
    displayName: "Fatou D.",
    role: "Prospection à Yamoussoukro",
  },
];

const AVATAR_COLORS = ["var(--palm)", "var(--gold)", "var(--clay)"];

export default function Presentation({ avis }: { avis: PublicAvis[] }) {
  const [toastVisible, setToastVisible] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const displayedAvis = avis.length > 0 ? avis : FALLBACK_AVIS;
...
@media (min-width:881px){
  .testi-grid.count-1{grid-template-columns:1fr;max-width:400px;margin:0 auto;}
  .testi-grid.count-2{grid-template-columns:repeat(2,1fr);max-width:700px;margin:0 auto;}
}
...
<section id="avis">
  <div className="wrap">
    ...
    <div className={`testi-grid reveal count-${Math.min(displayedAvis.length, 3)}`}>
      {displayedAvis.map((a, i) => (
        <div className="testi-card" key={i}>
          <div className="testi-quote">« {a.comment} »</div>
          <div className="testi-who">
            <div
              className="testi-avatar"
              style={{
                background: AVATAR_COLORS[i % AVATAR_COLORS.length],
                color: AVATAR_COLORS[i % AVATAR_COLORS.length] === "var(--gold)" ? "var(--forest-deep)" : "#fff",
              }}
            >
              {a.displayName[0]}
            </div>
            <div><div className="testi-name">{a.displayName}</div><div className="testi-role">{a.role}</div></div>
          </div>
        </div>
      ))}
    </div>
